...
func Handler(ctx context.Context, dbpool *pgxpool.Pool) *http.ServeMux {
	...
	plansHandler := newPlansHandler(createPlanUseCase, getPlanUseCase, updatePlanUseCase, deletePlanUseCase)
	baselinesHandler := newBaselinesHandler(createBaselineUseCase, updateBaselineUseCase, deleteBaselineUseCase)
	...
	r.HandleFunc("PATCH /costs/{costID}", costsHandler.updateCost)
	r.HandleFunc("DELETE /costs/{costID}", costsHandler.deleteCost)
